...
const getUnequiePeoples = function (peoples) {
    ...
    peoples.forEach(people => {
        if (uneqPeoples.findIndex(element => isEqual(element, people)) == -1) {
            uneqPeoples.push(people);
        }
    });
...
/**
 * Compare two any value, including objects
 * @param {any} a 
 * @param {any} b 
 * @return {boolean} Returns true if a deeply equals to b
 */
const isEqual = function (a, b) {

    if (a === b) return true;

    // a and b is simple type, or one of them is falsy
    if (!a || !b || (typeof a !== 'object' && typeof b !== 'object')) return a === b;

    // a and b are a differend objects
    if (a.prototype != b.prototype) return false;

    // array with properties of object a
    let keys = Object.keys(a);

    // objects have different keys count
    if (keys.length != Object.keys(b).length) return false;

    // check if every objects keys are equals
    return keys.every(property => isEqual(a[property], b[property]));
}

//console.log(isEqual(peoples[1],peoples[3])); // must be true
