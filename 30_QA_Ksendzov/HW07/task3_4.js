...
function printStructure(company, level = 0) {
  for (let parent of company) {

    placeholder = '--'.repeat(level).concat(' ').trimStart();
    console.log(`${placeholder + parent.name} (${parent.users_count})`);

    if (parent.children && parent.children.length){
      printStructure(parent.children, level+1);
    }
  }
}

printStructure(company);